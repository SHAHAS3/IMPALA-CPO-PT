CREATE OR REPLACE PACKAGE SYS_CPO_AUDIT AS
...
CREATE OR REPLACE PACKAGE BODY SYS_CPO_AUDIT AS
--
...
/*SELECT NVL (cols.column_name, NULL)
 ...
 WHERE cons.constraint_type = 'P'
   ...
   for c1 in (select * from SYS_AUDIT_COLUMNS_CPO where table_name=v_table_name) loop
   ...
   p_sql := p_sql || '   end if;';
   for c1 in (select a.*,b.data_type from SYS_AUDIT_COLUMNS_CPO a,USER_TAB_COLUMNS b  
