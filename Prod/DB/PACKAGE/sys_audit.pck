...
BEGIN
   ...
   for c1 in (select a.*,b.data_type from SYS_AUDIT_COLUMNS a,USER_TAB_COLUMNS b  
      ...
      if c1.data_type in('DATE','TIMESTAMP(6)') then
         p_sql := p_sql || '    values ('||g_seq_name||'.nextval,'||c1.id||','||g_user_id||',p_type,p_time,to_char(:OLD.'||c1.column_name||',''dd.mm.yyyy hh24:mi:ss''),to_char(:NEW.'||c1.column_name||',''dd.mm.yyyy hh24:mi:ss''),'||g_id1||','||g_id2||','||SYS_AUDIT.G_USER_NAME_COLUMN||');';
      else
         p_sql := p_sql || '    values ('||g_seq_name||'.nextval,'||c1.id||','||g_user_id||',p_type,p_time,:OLD.'||c1.column_name||',:NEW.'||c1.column_name||','||g_id1||','||g_id2||','||SYS_AUDIT.G_USER_NAME_COLUMN||');';
      end if;
      p_sql := p_sql || ' end if;';
   end loop;
   p_sql := p_sql || ' end;';    
