...
if (p_type = 'TPH') then
   ...
   if ( upper(substr(p_clinvalue,length(p_clinvalue),length(p_clinvalue))) = 'S'and (instr(upper(p_cprvalue),upper(p_clinvalue))>0 or instr(upper(p_clinvalue),upper(p_cprvalue))>0)  ) then
      v_result:='Y';
   end if;
   if (v_result = 'N' and  upper(substr(p_clinvalue,length(p_clinvalue),length(p_clinvalue))) = 'S' and instr(upper(p_cprvalue),upper(p_clinvalue))>0) then
      v_result:='Y';
   end if;
